                                     ...
                                     rocblas_int ldb)
{
    ...
    PRINT_IF_HIP_ERROR(
        hipMemsetAsync(invA, 0, BLOCK * BLOCK * *(handle->get_trsm_A_blks()) * sizeof(T), rocblas_stream));
    ...
    int W             = 1 + ((bsize - 1) / *(handle->get_trsm_B_chnk()));

    for(int w = 0; w < W; w++)
    {
        if(side == rocblas_side_left)
        {
            T* Bw = B + ((size_t)w) * *(handle->get_trsm_B_chnk()) * ((size_t)ldb);
            int width =
                ((bsize > (w + 1) * *(handle->get_trsm_B_chnk())) ? *(handle->get_trsm_B_chnk())
                                                         : (bsize - w * *(handle->get_trsm_B_chnk())));
        ...
        {
            T* Bw = B + ((size_t)w) * *(handle->get_trsm_B_chnk());
            int width =
                ((bsize > (w + 1) * *(handle->get_trsm_B_chnk())) ? *(handle->get_trsm_B_chnk())
                                                         : (bsize - w * *(handle->get_trsm_B_chnk())));
